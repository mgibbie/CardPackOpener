...
// Which art a card wears. A card generated by another (a Twinspell's second cast,
// an upgrade tier) has no illustration of its own — `artFrom` points at the card
// that makes it, so the pair reads as one card instead of the copy dropping to the
// procedural fallback. Falls back to the card's own id, which is the normal case.
export const artIdOf = card => (card && card.artFrom) || (card && card.id) || '';
function artFor(id) {
...
function paintArt(ctx, card, x, y, w, h, tuning) {
	...
	const artId = artIdOf(card); // a generated copy borrows its creator's illustration
	const img = artFor(artId);
	if (img) {
		const t = tuning !== undefined ? tuning : (ART_TUNING[card.id] ?? ART_TUNING[artId]);
...
export function drawCardFace(card, opts = {}) {
	...
	if (card.passive) {
		// passive treasures have no mana cost — no gem, and no top ribbon either
		// (owner call): the bottom tribe plate says "Passive" instead, so the
		// gold treasure frame + plate carry the identity without covering the art
	} else {
	...
	if (opts.goal != null) {
		...
		ctx.textAlign = 'left';
	} else if (card.passive || card.equip || card.tribe || card.type === 'weapon' || card.type === 'location' || card.type === 'enchantment' || card.type === 'artifact' || card.type === 'quest' || SPELL_TYPES_SET.has(card.type)) {
