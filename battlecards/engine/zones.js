...
export function drawCards(state, pi, count) {
	...
	for (let i = 0; i < count; i++) {
		...
		if (p.deckIdBuffs && p.deckIdBuffs.length) { const bi = p.deckIdBuffs.findIndex(b => b.id === id); if (bi >= 0) { const b = p.deckIdBuffs.splice(bi, 1)[0]; card.attack += b.attack || 0; card.maxHealth += b.health || 0; if (b.durability) card.durability = (card.durability || 0) + b.durability; if (b.cost) { card.cost = Math.max(0, (card.cost || 0) + b.cost); emit(state, { type: 'costChange', player: pi, uid: card.uid, cost: card.cost }); } } } // Beanstalk Brute: specific deck cards carry buffs; Blacksmithing Hammer: +Durability; Harpoon Gun: Beast -Cost
