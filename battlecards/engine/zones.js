...
export function drawCards(state, pi, count) {
	...
	for (let i = 0; i < count; i++) {
		...
		if (p.deckChooseBoth && p.deckChooseBoth[card.id] > 0 && card.choices) { card._chooseBoth = true; p.deckChooseBoth[card.id]--; } // Marvelous Mycelium (Duels): the shuffled pick resolves with both effects
		...
		if (p.deckIdBuffs && p.deckIdBuffs.length) { const bi = p.deckIdBuffs.findIndex(b => b.id === id); if (bi >= 0) { const b = p.deckIdBuffs.splice(bi, 1)[0]; card.attack += b.attack || 0; card.maxHealth += b.health || 0; if (b.durability) card.durability = (card.durability || 0) + b.durability; if (b.cost) { card.cost = Math.max(0, (card.cost || 0) + b.cost); emit(state, { type: 'costChange', player: pi, uid: card.uid, cost: card.cost }); } if (b.keywords) for (const kw of b.keywords) if (!card.keywords.includes(kw)) card.keywords.push(kw); } } // Beanstalk Brute: buffs; Blacksmithing Hammer: +Durability; Harpoon Gun: Beast -Cost; Kingsbane: kept enchantments
		// deck rewrites — the deck is ids, so these apply as cards take on a live identity
		if (card.type === 'creature' && p.deckStatsEqualCost) { const n = card.cost || 0; card.attack = n; card.maxHealth = n; } // Blackrock 'n' Roll
		if (card.type === 'creature' && p.deckSwapCostAttack) { const a = card.attack || 0, cc = card.cost || 0; card.attack = cc; card.cost = a; emit(state, { type: 'costChange', player: pi, uid: card.uid, cost: card.cost }); } // Deck of Chaos
		if (card.type === 'creature' && p.deckSetCost != null) { card.cost = p.deckSetCost; emit(state, { type: 'costChange', player: pi, uid: card.uid, cost: card.cost }); } // The Fires of Zin-Azshari
