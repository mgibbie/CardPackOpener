...
export function effectiveCost(state, pi, card) {
	...
	if (card.selfCostIf) {
		...
		switch (s.cond) {
			...
			case 'enemy-copy-while-held': met = !!card._enemyCopyWhileHeld; break; // Unshackle Soul: you played a copy of an opponent's card while holding this
	...
	if (isSpellType(card) && ((p.spellsPlayedThisTurn || 0) + 1) % 3 === 0 && p.board.some(x => x.scionThird && !isDead(x))) c = 0; // Scion of the Deep (Duels): the third spell is paid in Health instead (playCard charges it)
	if (isSpellType(card) && /Eclipse/.test(card.name || '') && p.board.some(x => x.eclipseFree && !isDead(x))) c = 0; // Moonbeast (Duels): your Eclipse spells cost (0)
	...
	if (p.nextSpellCostsHealthTurn === state.turnNumber && isSpellType(card)) return 0; // Bloodbloom: the next spell this turn is paid in Health
	if (p.minionsCostHealthTurn === state.turnNumber && card.type === 'creature') return 0; // Curse of Flesh: your minions cost Health this turn
