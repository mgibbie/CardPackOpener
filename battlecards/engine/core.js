...
export function instantiate(def, controller) {
	const card = {
		...
		statics: def.statics ? def.statics.map(s => ({ ...s })) : null, // multiple passives on one card (Xixira)
...
export function playCard(state, pi, cardUid, target, choice, position, useAlt, kicked) {
	...
	if (card.type === 'creature' && card.magnetic && target?.type === 'creature'
		...
		{ const sch = schoolOf(card); if (sch) { (p.schoolsCastThisTurn = p.schoolsCastThisTurn || {})[sch] = true; (p.schoolsCastGame = p.schoolsCastGame || {})[sch] = true; if (sch === 'Fel') (p.felSpellsGame = p.felSpellsGame || []).push(card.id); if (sch === 'Frost') p.frostSpellsGame = (p.frostSpellsGame || 0) + 1; if (sch === 'Holy' && p.holyRefreshShield && !p.heroShield) { p.heroShield = true; emit(state, { type: 'heroShield', player: pi }); } } } // Metamorfin / Multicaster / Jace / Bearon; Starlight Groove: Holy spells refresh Divine Shield
...
function applyForge(state, pi, card) {
	...
	if (f.miracle) card.miracle = JSON.parse(JSON.stringify(f.miracle)); // replace the Miracle/Quickdraw effect
	if (f.clearMiracle) card.miracle = null; // Glowstone Gyreworm: Quickdraw becomes a Battlecry (drop the draw-gated version)
...
export { SCHEMA_VERSION, toSnapshot, fromSnapshot, migrate, normalize, stateDigest, maxSnapshotUid } from './serialize.js';
