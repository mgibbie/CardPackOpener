...
// the collection a fresh account is handed: 2x each common/basic (or no rarity), 1x each uncommon
const newAccountCollection = {};
for (const d of Object.values(cardsById)) {
	if (!Col.collectible(d)) continue;
	const r = d.rarity;
	if (!r || r === 'common' || r === 'basic') newAccountCollection[d.id] = 2;
	else if (r === 'uncommon') newAccountCollection[d.id] = 1;
}
