...
// --- registry contract ---
{
	ok('pilot + batch types registered (1120 incl. harvest + untap-land + transform-same-cost + advance + assemble + roll-scry + add-random-card + destroy-permanent + vote + grant/target/buff/swap picks)', registeredTypes().length === 1120 && !!getEffectHandler('harvest') && !!getEffectHandler('untap-land') && !!getEffectHandler('transform-same-cost') && !!getEffectHandler('vote') && !!getEffectHandler('add-random-card') && !!getEffectHandler('destroy-permanent') && !!getEffectHandler('roll-scry') && !!getEffectHandler('armor') && !!getEffectHandler('spend-all-mana-damage') && !!getEffectHandler('advance') && !!getEffectHandler('assemble') && !!getEffectHandler('grant-target') && !!getEffectHandler('target-player') && !!getEffectHandler('buff-target') && !!getEffectHandler('swap-stats-cross'));
