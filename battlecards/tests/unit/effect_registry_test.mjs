...
// --- registry contract ---
{
	ok('pilot + batch types registered (1202 incl. summon-attackers + grow + weaken + destroy-parity + copy-enemy-location + stats-equal-x + sacrifice-friendly + graveyard-vote + fight-random-beast + destroy-walker + reveal-tax-draw + buff-target-and-adjacent + destroy-target-and-adjacent + exile-graveyards + regenerate + sac-land + exile-graveyard-random + skip-enemy-turn + harvest + untap-land + transform-same-cost + advance + assemble + tempt + roll-scry + roll-d6 + add-random-card + destroy-permanent + vote + grant/target/buff/swap picks + the Violet Hold Follow chain + the Duels active-treasure + hero-power + skips + arsenal waves)', registeredTypes().length === 1202 && !!getEffectHandler('tempt') && !!getEffectHandler('grow') && !!getEffectHandler('weaken') && !!getEffectHandler('destroy-parity') && !!getEffectHandler('copy-enemy-location') && !!getEffectHandler('stats-equal-x') && !!getEffectHandler('sacrifice-friendly') && !!getEffectHandler('graveyard-vote') && !!getEffectHandler('fight-random-beast') && !!getEffectHandler('destroy-walker') && !!getEffectHandler('reveal-tax-draw') && !!getEffectHandler('buff-target-and-adjacent') && !!getEffectHandler('destroy-target-and-adjacent') && !!getEffectHandler('exile-graveyards') && !!getEffectHandler('regenerate') && !!getEffectHandler('sac-land') && !!getEffectHandler('exile-graveyard-random') && !!getEffectHandler('skip-enemy-turn') && !!getEffectHandler('harvest') && !!getEffectHandler('untap-land') && !!getEffectHandler('transform-same-cost') && !!getEffectHandler('vote') && !!getEffectHandler('add-random-card') && !!getEffectHandler('destroy-permanent') && !!getEffectHandler('roll-scry') && !!getEffectHandler('roll-d6') && !!getEffectHandler('armor') && !!getEffectHandler('spend-all-mana-damage') && !!getEffectHandler('advance') && !!getEffectHandler('assemble') && !!getEffectHandler('grant-target') && !!getEffectHandler('target-player') && !!getEffectHandler('buff-target') && !!getEffectHandler('swap-stats-cross') && !!getEffectHandler('follow-echo') && !!getEffectHandler('silent-strike') && !!getEffectHandler('tricks-of-the-trade') && !!getEffectHandler('slime-board') && !!getEffectHandler('soul-immolation') && !!getEffectHandler('desperate-bribe') && !!getEffectHandler('transform-all-friendly-costplus') && !!getEffectHandler('discover-enemy-deck-to-top') && !!getEffectHandler('summon-attackers'));
