...
// --- registry contract ---
{
	ok('pilot + batch types registered (1124 incl. regenerate + sac-land + exile-graveyard-random + skip-enemy-turn + harvest + untap-land + transform-same-cost + advance + assemble + roll-scry + add-random-card + destroy-permanent + vote + grant/target/buff/swap picks)', registeredTypes().length === 1124 && !!getEffectHandler('regenerate') && !!getEffectHandler('sac-land') && !!getEffectHandler('exile-graveyard-random') && !!getEffectHandler('skip-enemy-turn') && !!getEffectHandler('harvest') && !!getEffectHandler('untap-land') && !!getEffectHandler('transform-same-cost') && !!getEffectHandler('vote') && !!getEffectHandler('add-random-card') && !!getEffectHandler('destroy-permanent') && !!getEffectHandler('roll-scry') && !!getEffectHandler('armor') && !!getEffectHandler('spend-all-mana-damage') && !!getEffectHandler('advance') && !!getEffectHandler('assemble') && !!getEffectHandler('grant-target') && !!getEffectHandler('target-player') && !!getEffectHandler('buff-target') && !!getEffectHandler('swap-stats-cross'));
