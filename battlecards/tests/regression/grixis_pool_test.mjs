// Grixis tri-color advanced-land pool: 15 three-color, uncollectible "Grixis" cards the land
// Discovers from (keyed by landSet:"Grixis", NOT name-match — the name-match conjure path
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Grixis' && !c.token);
ok('at least 15 Grixis pool cards', pool.length >= 15, pool.length);
ok('all Grixis cards uncollectible', pool.every(c => c.collectible === false));
ok('all Grixis cards tri-color', pool.every(c => JSON.stringify(c.colors) === '["B","R","U"]'));
ok('all Grixis cards tagged landSet Grixis', pool.every(c => c.landSet === 'Grixis'));

// ---- land tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.grixis_panorama;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Grixis');
ok('land tap keys off landSet Grixis', tapEff && tapEff.landSet === 'Grixis', JSON.stringify(tapEff));
...
// ---- discover from the land pool offers only Grixis cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Grixis' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Grixis cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Grixis'), pend && pend.ids);
}
