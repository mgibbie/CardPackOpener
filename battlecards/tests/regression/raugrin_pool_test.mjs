// Raugrin tri-color advanced-land pool: three-color, uncollectible "Raugrin" cards the land
// Discovers from (keyed by landSet:"Raugrin"). Pools are a 15-card MINIMUM, not a cap.
import fs from 'fs';
...
const pool = raw.cards.filter(c => c.landSet === 'Raugrin' && !c.token);
ok('at least 15 Raugrin pool cards', pool.length >= 15, pool.length);
ok('all Raugrin cards uncollectible', pool.every(c => c.collectible === false));
ok('all Raugrin cards tri-color', pool.every(c => JSON.stringify(c.colors) === '["W","U","R"]'));
ok('all Raugrin cards tagged landSet Raugrin', pool.every(c => c.landSet === 'Raugrin'));

const temple = byId.raugrin_triome;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Raugrin');
ok('land tap keys off landSet Raugrin', tapEff && tapEff.landSet === 'Raugrin', JSON.stringify(tapEff));
...
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Raugrin' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Raugrin cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Raugrin'), pend && pend.ids);
}
