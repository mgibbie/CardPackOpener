// Bretagard Citadel advanced-land pool: 15 dual-color, uncollectible "Bretagard" cards the land
// Discovers from (keyed by landSet:"Bretagard", NOT name-match — the name-match conjure path
...
const temple = byId.bretagard_citadel;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Bretagard');
ok('land tap keys off landSet Bretagard', tapEff && tapEff.landSet === 'Bretagard', JSON.stringify(tapEff));
...
// ---- discover from the land pool offers only Bretagard cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Bretagard' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Bretagard cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Bretagard'), pend && pend.ids);
}
