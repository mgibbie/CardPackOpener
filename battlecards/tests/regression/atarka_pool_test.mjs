// Temple of Atarka advanced-land pool: 15 mono-white, uncollectible "Atarka" cards the
// land Discovers from (keyed by landSet:"Atarka", NOT name-match — the name-match conjure
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Atarka' && !c.token);
ok('at least 15 Atarka pool cards', pool.length >= 15, pool.length);
ok('all Atarka cards uncollectible', pool.every(c => c.collectible === false));
ok('all Atarka cards dual-color', pool.every(c => JSON.stringify(c.colors) === '[\"R\",\"G\"]'));
ok('all Atarka names contain "Atarka"', pool.every(c => c.name.includes('Atarka')),
	pool.filter(c => !c.name.includes('Atarka')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.atarka_dragonspire;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Atarka');
ok('temple tap keys off landSet Atarka', tapEff && tapEff.landSet === 'Atarka', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Atarka cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Atarka' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Atarka cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Atarka'), pend && pend.ids);
}
