// Istfell Citadel advanced-land pool: 15 dual-color, uncollectible "Istfell" cards the land
// Discovers from (keyed by landSet:"Istfell", NOT name-match — the name-match conjure path
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Istfell' && !c.token);
ok('at least 15 Istfell pool cards', pool.length >= 15, pool.length);
ok('all Istfell cards uncollectible', pool.every(c => c.collectible === false));
ok('all Istfell cards dual-color', pool.every(c => JSON.stringify(c.colors) === '["W","U"]'));
ok('all Istfell cards tagged landSet Istfell', pool.every(c => c.landSet === 'Istfell'));

// ---- land tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.istfell_citadel;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Istfell');
ok('land tap keys off landSet Istfell', tapEff && tapEff.landSet === 'Istfell', JSON.stringify(tapEff));
...
// ---- discover from the land pool offers only Istfell cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Istfell' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Istfell cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Istfell'), pend && pend.ids);
}
