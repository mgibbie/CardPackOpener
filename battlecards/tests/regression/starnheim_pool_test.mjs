// Starnheim Citadel advanced-land pool: 15 dual-color, uncollectible "Starnheim" cards the land
// Discovers from (keyed by landSet:"Starnheim", NOT name-match — the name-match conjure path
...
const temple = byId.starnheim_citadel;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Starnheim');
ok('land tap keys off landSet Starnheim', tapEff && tapEff.landSet === 'Starnheim', JSON.stringify(tapEff));
...
// ---- discover from the land pool offers only Starnheim cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Starnheim' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Starnheim cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Starnheim'), pend && pend.ids);
}
