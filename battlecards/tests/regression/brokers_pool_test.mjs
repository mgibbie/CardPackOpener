// Brokers tri-color advanced-land pool: three-color, uncollectible "Brokers" cards the land
// Discovers from (keyed by landSet:"Brokers"). Pools are a 15-card MINIMUM, not a cap.
import fs from 'fs';
...
const pool = raw.cards.filter(c => c.landSet === 'Brokers' && !c.token);
ok('at least 15 Brokers pool cards', pool.length >= 15, pool.length);
ok('all Brokers cards uncollectible', pool.every(c => c.collectible === false));
ok('all Brokers cards tri-color', pool.every(c => JSON.stringify(c.colors) === '["W","U","G"]'));
ok('all Brokers cards tagged landSet Brokers', pool.every(c => c.landSet === 'Brokers'));

const temple = byId.brokers_hideout;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Brokers');
ok('land tap keys off landSet Brokers', tapEff && tapEff.landSet === 'Brokers', JSON.stringify(tapEff));
...
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Brokers' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Brokers cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Brokers'), pend && pend.ids);
}
