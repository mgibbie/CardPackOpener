// Temple of Kolaghan advanced-land pool: 15 mono-white, uncollectible "Kolaghan" cards the
// land Discovers from (keyed by landSet:"Kolaghan", NOT name-match — the name-match conjure
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Kolaghan' && !c.token);
ok('at least 15 Kolaghan pool cards', pool.length >= 15, pool.length);
ok('all Kolaghan cards uncollectible', pool.every(c => c.collectible === false));
ok('all Kolaghan cards dual-color', pool.every(c => JSON.stringify(c.colors) === '[\"B\",\"R\"]'));
ok('all Kolaghan names contain "Kolaghan"', pool.every(c => c.name.includes('Kolaghan')),
	pool.filter(c => !c.name.includes('Kolaghan')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.kolaghan_dragonspire;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Kolaghan');
ok('temple tap keys off landSet Kolaghan', tapEff && tapEff.landSet === 'Kolaghan', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Kolaghan cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Kolaghan' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Kolaghan cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Kolaghan'), pend && pend.ids);
}
