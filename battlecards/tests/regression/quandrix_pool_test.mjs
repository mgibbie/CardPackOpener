// Temple of Quandrix advanced-land pool: 15 mono-white, uncollectible "Quandrix" cards the
// land Discovers from (keyed by landSet:"Quandrix", NOT name-match — the name-match conjure
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Quandrix' && !c.token);
ok('at least 15 Quandrix pool cards', pool.length >= 15, pool.length);
ok('all Quandrix cards uncollectible', pool.every(c => c.collectible === false));
ok('all Quandrix cards dual-color', pool.every(c => JSON.stringify(c.colors) === '[\"G\",\"U\"]'));
ok('all Quandrix cards tagged landSet Quandrix', pool.every(c => c.landSet === 'Quandrix'));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.quandrix_campus;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Quandrix');
ok('temple tap keys off landSet Quandrix', tapEff && tapEff.landSet === 'Quandrix', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Quandrix cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Quandrix' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Quandrix cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Quandrix'), pend && pend.ids);
}
