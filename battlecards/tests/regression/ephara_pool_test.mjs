// Temple of Ephara advanced-land pool: 15 mono-white, uncollectible "Ephara" cards the
// land Discovers from (keyed by landSet:"Ephara", NOT name-match — the name-match conjure
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Ephara' && !c.token);
ok('at least 15 Ephara pool cards', pool.length >= 15, pool.length);
ok('all Ephara cards uncollectible', pool.every(c => c.collectible === false));
ok('all Ephara cards dual-color', pool.every(c => JSON.stringify(c.colors) === '[\"W\",\"U\"]'));
ok('all Ephara names contain "Ephara"', pool.every(c => c.name.includes('Ephara')),
	pool.filter(c => !c.name.includes('Ephara')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.temple_of_ephara;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Ephara');
ok('temple tap keys off landSet Ephara', tapEff && tapEff.landSet === 'Ephara', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Ephara cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Ephara' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Ephara cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Ephara'), pend && pend.ids);
}
