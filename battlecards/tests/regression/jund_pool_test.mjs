// Jund tri-color advanced-land pool: 15 three-color, uncollectible "Jund" cards the land
// Discovers from (keyed by landSet:"Jund", NOT name-match — the name-match conjure path
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Jund' && !c.token);
ok('at least 15 Jund pool cards', pool.length >= 15, pool.length);
ok('all Jund cards uncollectible', pool.every(c => c.collectible === false));
ok('all Jund cards tri-color', pool.every(c => JSON.stringify(c.colors) === '["B","G","R"]'));
ok('all Jund cards tagged landSet Jund', pool.every(c => c.landSet === 'Jund'));

// ---- land tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.jund_panorama;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Jund');
ok('land tap keys off landSet Jund', tapEff && tapEff.landSet === 'Jund', JSON.stringify(tapEff));
...
// ---- discover from the land pool offers only Jund cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Jund' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Jund cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Jund'), pend && pend.ids);
}
