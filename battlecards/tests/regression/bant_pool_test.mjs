// Bant tri-color advanced-land pool: 15 three-color, uncollectible "Bant" cards the land
// Discovers from (keyed by landSet:"Bant", NOT name-match — the name-match conjure path
...
const temple = byId.bant_citadel;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Bant');
ok('land tap keys off landSet Bant', tapEff && tapEff.landSet === 'Bant', JSON.stringify(tapEff));
...
// ---- discover from the land pool offers only Bant cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Bant' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Bant cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Bant'), pend && pend.ids);
}
