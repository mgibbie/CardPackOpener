// Temple of Erebos advanced-land pool: 15 mono-white, uncollectible "Erebos" cards the
// land Discovers from (keyed by landSet:"Erebos", NOT name-match — the name-match conjure
...
ok('all Erebos names contain "Erebos"', pool.every(c => c.name.includes('Erebos')),
	pool.filter(c => !c.name.includes('Erebos')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.temple_of_erebos;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Erebos');
ok('temple tap keys off landSet Erebos', tapEff && tapEff.landSet === 'Erebos', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Erebos cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Erebos' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Erebos cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Erebos'), pend && pend.ids);
}
