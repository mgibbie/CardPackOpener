// Temple of Kefnet advanced-land pool: 15 mono-white, uncollectible "Kefnet" cards the
// land Discovers from (keyed by landSet:"Kefnet", NOT name-match — the name-match conjure
...
const temple = byId.temple_of_kefnet;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Kefnet');
ok('temple tap keys off landSet Kefnet', tapEff && tapEff.landSet === 'Kefnet', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Kefnet cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Kefnet' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Kefnet cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Kefnet'), pend && pend.ids);
}
