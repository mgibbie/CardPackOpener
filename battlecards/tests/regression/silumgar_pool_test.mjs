// Temple of Silumgar advanced-land pool: 15 mono-white, uncollectible "Silumgar" cards the
// land Discovers from (keyed by landSet:"Silumgar", NOT name-match — the name-match conjure
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Silumgar' && !c.token);
ok('at least 15 Silumgar pool cards', pool.length >= 15, pool.length);
ok('all Silumgar cards uncollectible', pool.every(c => c.collectible === false));
ok('all Silumgar cards dual-color', pool.every(c => JSON.stringify(c.colors) === '[\"U\",\"B\"]'));
ok('all Silumgar names contain "Silumgar"', pool.every(c => c.name.includes('Silumgar')),
	pool.filter(c => !c.name.includes('Silumgar')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.silumgar_dragonspire;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Silumgar');
ok('temple tap keys off landSet Silumgar', tapEff && tapEff.landSet === 'Silumgar', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Silumgar cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Silumgar' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Silumgar cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Silumgar'), pend && pend.ids);
}
