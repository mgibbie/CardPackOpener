// Ketria tri-color advanced-land pool: three-color, uncollectible "Ketria" cards the land
// Discovers from (keyed by landSet:"Ketria"). Pools are a 15-card MINIMUM, not a cap.
import fs from 'fs';
...
const pool = raw.cards.filter(c => c.landSet === 'Ketria' && !c.token);
ok('at least 15 Ketria pool cards', pool.length >= 15, pool.length);
ok('all Ketria cards uncollectible', pool.every(c => c.collectible === false));
ok('all Ketria cards tri-color', pool.every(c => JSON.stringify(c.colors) === '["U","R","G"]'));
ok('all Ketria cards tagged landSet Ketria', pool.every(c => c.landSet === 'Ketria'));

const temple = byId.ketria_triome;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Ketria');
ok('land tap keys off landSet Ketria', tapEff && tapEff.landSet === 'Ketria', JSON.stringify(tapEff));
...
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Ketria' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Ketria cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Ketria'), pend && pend.ids);
}
