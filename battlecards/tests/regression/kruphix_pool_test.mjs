// Temple of Kruphix advanced-land pool: 15 mono-white, uncollectible "Kruphix" cards the
// land Discovers from (keyed by landSet:"Kruphix", NOT name-match — the name-match conjure
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Kruphix' && !c.token);
ok('at least 15 Kruphix pool cards', pool.length >= 15, pool.length);
ok('all Kruphix cards uncollectible', pool.every(c => c.collectible === false));
ok('all Kruphix cards dual-color', pool.every(c => JSON.stringify(c.colors) === '[\"G\",\"U\"]'));
ok('all Kruphix names contain "Kruphix"', pool.every(c => c.name.includes('Kruphix')),
	pool.filter(c => !c.name.includes('Kruphix')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.temple_of_kruphix;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Kruphix');
ok('temple tap keys off landSet Kruphix', tapEff && tapEff.landSet === 'Kruphix', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Kruphix cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Kruphix' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Kruphix cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Kruphix'), pend && pend.ids);
}
