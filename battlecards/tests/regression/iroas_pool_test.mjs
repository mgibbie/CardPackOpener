// Temple of Iroas advanced-land pool: 15 mono-white, uncollectible "Iroas" cards the
// land Discovers from (keyed by landSet:"Iroas", NOT name-match — the name-match conjure
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Iroas' && !c.token);
ok('at least 15 Iroas pool cards', pool.length >= 15, pool.length);
ok('all Iroas cards uncollectible', pool.every(c => c.collectible === false));
ok('all Iroas cards dual-color', pool.every(c => JSON.stringify(c.colors) === '[\"R\",\"W\"]'));
ok('all Iroas names contain "Iroas"', pool.every(c => c.name.includes('Iroas')),
	pool.filter(c => !c.name.includes('Iroas')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.temple_of_iroas;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Iroas');
ok('temple tap keys off landSet Iroas', tapEff && tapEff.landSet === 'Iroas', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Iroas cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Iroas' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Iroas cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Iroas'), pend && pend.ids);
}
