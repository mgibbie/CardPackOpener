// Karfell Citadel advanced-land pool: 15 dual-color, uncollectible "Karfell" cards the land
// Discovers from (keyed by landSet:"Karfell", NOT name-match — the name-match conjure path
...
const temple = byId.karfell_citadel;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Karfell');
ok('land tap keys off landSet Karfell', tapEff && tapEff.landSet === 'Karfell', JSON.stringify(tapEff));
...
// ---- discover from the land pool offers only Karfell cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Karfell' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Karfell cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Karfell'), pend && pend.ids);
}
