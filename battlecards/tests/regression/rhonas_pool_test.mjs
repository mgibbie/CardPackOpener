// Temple of Rhonas advanced-land pool: 15 mono-white, uncollectible "Rhonas" cards the
// land Discovers from (keyed by landSet:"Rhonas", NOT name-match — the name-match conjure
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Rhonas' && !c.token);
ok('at least 15 Rhonas pool cards', pool.length >= 15, pool.length);
ok('all Rhonas cards uncollectible', pool.every(c => c.collectible === false));
ok('all Rhonas cards mono-white', pool.every(c => JSON.stringify(c.colors) === '["G"]'));
ok('all Rhonas names contain "Rhonas"', pool.every(c => c.name.includes('Rhonas')),
	pool.filter(c => !c.name.includes('Rhonas')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.temple_of_rhonas;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Rhonas');
ok('temple tap keys off landSet Rhonas', tapEff && tapEff.landSet === 'Rhonas', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Rhonas cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Rhonas' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Rhonas cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Rhonas'), pend && pend.ids);
}
