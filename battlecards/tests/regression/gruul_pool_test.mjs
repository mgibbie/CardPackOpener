// Temple of Gruul advanced-land pool: 15 mono-white, uncollectible "Gruul" cards the
// land Discovers from (keyed by landSet:"Gruul", NOT name-match — the name-match conjure
...
ok('all Gruul names contain "Gruul"', pool.every(c => c.name.includes('Gruul')),
	pool.filter(c => !c.name.includes('Gruul')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.gruul_guildgate;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Gruul');
ok('temple tap keys off landSet Gruul', tapEff && tapEff.landSet === 'Gruul', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Gruul cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Gruul' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Gruul cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Gruul'), pend && pend.ids);
}
