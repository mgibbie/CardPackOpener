// Temple of Heliod advanced-land pool: 15 mono-white, uncollectible "Heliod" cards the
// land Discovers from (keyed by landSet:"Heliod", NOT name-match — the name-match conjure
...
ok('all Heliod names contain "Heliod"', pool.every(c => c.name.includes('Heliod')),
	pool.filter(c => !c.name.includes('Heliod')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.temple_of_heliod;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Heliod');
ok('temple tap keys off landSet Heliod', tapEff && tapEff.landSet === 'Heliod', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Heliod cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Heliod' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Heliod cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Heliod'), pend && pend.ids);
}
