// Jeskai tri-color advanced-land pool: 15 three-color, uncollectible "Jeskai" cards the land
// Discovers from (keyed by landSet:"Jeskai", NOT name-match — the name-match conjure path
...
const temple = byId.jeskai_citadel;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Jeskai');
ok('land tap keys off landSet Jeskai', tapEff && tapEff.landSet === 'Jeskai', JSON.stringify(tapEff));
...
// ---- discover from the land pool offers only Jeskai cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Jeskai' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Jeskai cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Jeskai'), pend && pend.ids);
}
