// Temple of Simic advanced-land pool: 15 mono-white, uncollectible "Simic" cards the
// land Discovers from (keyed by landSet:"Simic", NOT name-match — the name-match conjure
...
ok('all Simic names contain "Simic"', pool.every(c => c.name.includes('Simic')),
	pool.filter(c => !c.name.includes('Simic')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.simic_guildgate;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Simic');
ok('temple tap keys off landSet Simic', tapEff && tapEff.landSet === 'Simic', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Simic cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Simic' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Simic cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Simic'), pend && pend.ids);
}
