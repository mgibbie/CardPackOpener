// Cabaretti tri-color advanced-land pool: three-color, uncollectible "Cabaretti" cards the land
// Discovers from (keyed by landSet:"Cabaretti"). Pools are a 15-card MINIMUM, not a cap.
import fs from 'fs';
...
const pool = raw.cards.filter(c => c.landSet === 'Cabaretti' && !c.token);
ok('at least 15 Cabaretti pool cards', pool.length >= 15, pool.length);
ok('all Cabaretti cards uncollectible', pool.every(c => c.collectible === false));
ok('all Cabaretti cards tri-color', pool.every(c => JSON.stringify(c.colors) === '["W","R","G"]'));
ok('all Cabaretti cards tagged landSet Cabaretti', pool.every(c => c.landSet === 'Cabaretti'));

const temple = byId.cabaretti_hideout;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Cabaretti');
ok('land tap keys off landSet Cabaretti', tapEff && tapEff.landSet === 'Cabaretti', JSON.stringify(tapEff));
...
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Cabaretti' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Cabaretti cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Cabaretti'), pend && pend.ids);
}
