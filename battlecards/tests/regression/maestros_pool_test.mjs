// Maestros tri-color advanced-land pool: three-color, uncollectible "Maestros" cards the land
// Discovers from (keyed by landSet:"Maestros"). Pools are a 15-card MINIMUM, not a cap.
import fs from 'fs';
...
const pool = raw.cards.filter(c => c.landSet === 'Maestros' && !c.token);
ok('at least 15 Maestros pool cards', pool.length >= 15, pool.length);
ok('all Maestros cards uncollectible', pool.every(c => c.collectible === false));
ok('all Maestros cards tri-color', pool.every(c => JSON.stringify(c.colors) === '["U","B","R"]'));
ok('all Maestros cards tagged landSet Maestros', pool.every(c => c.landSet === 'Maestros'));

const temple = byId.maestros_headquarters;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Maestros');
ok('land tap keys off landSet Maestros', tapEff && tapEff.landSet === 'Maestros', JSON.stringify(tapEff));
...
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Maestros' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Maestros cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Maestros'), pend && pend.ids);
}
