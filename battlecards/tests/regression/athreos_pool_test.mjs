// Temple of Athreos advanced-land pool: 15 mono-white, uncollectible "Athreos" cards the
// land Discovers from (keyed by landSet:"Athreos", NOT name-match — the name-match conjure
...
ok('all Athreos names contain "Athreos"', pool.every(c => c.name.includes('Athreos')),
	pool.filter(c => !c.name.includes('Athreos')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.temple_of_athreos;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Athreos');
ok('temple tap keys off landSet Athreos', tapEff && tapEff.landSet === 'Athreos', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Athreos cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Athreos' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Athreos cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Athreos'), pend && pend.ids);
}
