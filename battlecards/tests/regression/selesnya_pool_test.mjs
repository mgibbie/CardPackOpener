// Temple of Selesnya advanced-land pool: 15 mono-white, uncollectible "Selesnya" cards the
// land Discovers from (keyed by landSet:"Selesnya", NOT name-match — the name-match conjure
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Selesnya' && !c.token);
ok('at least 15 Selesnya pool cards', pool.length >= 15, pool.length);
ok('all Selesnya cards uncollectible', pool.every(c => c.collectible === false));
ok('all Selesnya cards are G/W dual-color', pool.every(c => JSON.stringify(c.colors) === '["G","W"]'));
ok('all Selesnya names contain "Selesnya"', pool.every(c => c.name.includes('Selesnya')),
	pool.filter(c => !c.name.includes('Selesnya')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.selesnya_guildgate;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Selesnya');
ok('temple tap keys off landSet Selesnya', tapEff && tapEff.landSet === 'Selesnya', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Selesnya cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Selesnya' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Selesnya cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Selesnya'), pend && pend.ids);
}
