// Savai tri-color advanced-land pool: three-color, uncollectible "Savai" cards the land
// Discovers from (keyed by landSet:"Savai"). Pools are a 15-card MINIMUM, not a cap.
import fs from 'fs';
...
const pool = raw.cards.filter(c => c.landSet === 'Savai' && !c.token);
ok('at least 15 Savai pool cards', pool.length >= 15, pool.length);
ok('all Savai cards uncollectible', pool.every(c => c.collectible === false));
ok('all Savai cards tri-color', pool.every(c => JSON.stringify(c.colors) === '["W","B","R"]'));
ok('all Savai cards tagged landSet Savai', pool.every(c => c.landSet === 'Savai'));

const temple = byId.savai_triome;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Savai');
ok('land tap keys off landSet Savai', tapEff && tapEff.landSet === 'Savai', JSON.stringify(tapEff));
...
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Savai' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Savai cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Savai'), pend && pend.ids);
}
