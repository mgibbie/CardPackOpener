// Temple of Bontu advanced-land pool: 15 mono-white, uncollectible "Bontu" cards the
// land Discovers from (keyed by landSet:"Bontu", NOT name-match — the name-match conjure
...
ok('all Bontu names contain "Bontu"', pool.every(c => c.name.includes('Bontu')),
	pool.filter(c => !c.name.includes('Bontu')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.temple_of_bontu;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Bontu');
ok('temple tap keys off landSet Bontu', tapEff && tapEff.landSet === 'Bontu', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Bontu cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Bontu' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Bontu cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Bontu'), pend && pend.ids);
}
