// Temple of Phenax advanced-land pool: 15 mono-white, uncollectible "Phenax" cards the
// land Discovers from (keyed by landSet:"Phenax", NOT name-match — the name-match conjure
...
ok('all Phenax names contain "Phenax"', pool.every(c => c.name.includes('Phenax')),
	pool.filter(c => !c.name.includes('Phenax')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.temple_of_phenax;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Phenax');
ok('temple tap keys off landSet Phenax', tapEff && tapEff.landSet === 'Phenax', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Phenax cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Phenax' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Phenax cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Phenax'), pend && pend.ids);
}
