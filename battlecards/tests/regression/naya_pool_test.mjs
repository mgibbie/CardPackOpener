// Naya tri-color advanced-land pool: 15 three-color, uncollectible "Naya" cards the land
// Discovers from (keyed by landSet:"Naya", NOT name-match — the name-match conjure path
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Naya' && !c.token);
ok('at least 15 Naya pool cards', pool.length >= 15, pool.length);
ok('all Naya cards uncollectible', pool.every(c => c.collectible === false));
ok('all Naya cards tri-color', pool.every(c => JSON.stringify(c.colors) === '["G","R","W"]'));
ok('all Naya cards tagged landSet Naya', pool.every(c => c.landSet === 'Naya'));

// ---- land tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.naya_panorama;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Naya');
ok('land tap keys off landSet Naya', tapEff && tapEff.landSet === 'Naya', JSON.stringify(tapEff));
...
// ---- discover from the land pool offers only Naya cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Naya' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Naya cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Naya'), pend && pend.ids);
}
