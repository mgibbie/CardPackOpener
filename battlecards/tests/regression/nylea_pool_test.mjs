// Temple of Nylea advanced-land pool: 15 mono-white, uncollectible "Nylea" cards the
// land Discovers from (keyed by landSet:"Nylea", NOT name-match — the name-match conjure
...
ok('all Nylea names contain "Nylea"', pool.every(c => c.name.includes('Nylea')),
	pool.filter(c => !c.name.includes('Nylea')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.temple_of_nylea;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Nylea');
ok('temple tap keys off landSet Nylea', tapEff && tapEff.landSet === 'Nylea', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Nylea cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Nylea' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Nylea cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Nylea'), pend && pend.ids);
}
