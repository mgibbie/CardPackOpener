// Temple of Dimir advanced-land pool: 15 mono-white, uncollectible "Dimir" cards the
// land Discovers from (keyed by landSet:"Dimir", NOT name-match — the name-match conjure
...
ok('all Dimir names contain "Dimir"', pool.every(c => c.name.includes('Dimir')),
	pool.filter(c => !c.name.includes('Dimir')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.dimir_guildgate;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Dimir');
ok('temple tap keys off landSet Dimir', tapEff && tapEff.landSet === 'Dimir', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Dimir cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Dimir' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Dimir cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Dimir'), pend && pend.ids);
}
