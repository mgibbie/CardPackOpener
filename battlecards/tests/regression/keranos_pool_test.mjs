// Temple of Keranos advanced-land pool: 15 mono-white, uncollectible "Keranos" cards the
// land Discovers from (keyed by landSet:"Keranos", NOT name-match — the name-match conjure
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Keranos' && !c.token);
ok('at least 15 Keranos pool cards', pool.length >= 15, pool.length);
ok('all Keranos cards uncollectible', pool.every(c => c.collectible === false));
ok('all Keranos cards dual-color', pool.every(c => JSON.stringify(c.colors) === '[\"U\",\"R\"]'));
ok('all Keranos names contain "Keranos"', pool.every(c => c.name.includes('Keranos')),
	pool.filter(c => !c.name.includes('Keranos')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.temple_of_keranos;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Keranos');
ok('temple tap keys off landSet Keranos', tapEff && tapEff.landSet === 'Keranos', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Keranos cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Keranos' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Keranos cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Keranos'), pend && pend.ids);
}
