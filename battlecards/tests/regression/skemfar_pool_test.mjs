// Skemfar Citadel advanced-land pool: 15 dual-color, uncollectible "Skemfar" cards the land
// Discovers from (keyed by landSet:"Skemfar", NOT name-match — the name-match conjure path
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Skemfar' && !c.token);
ok('at least 15 Skemfar pool cards', pool.length >= 15, pool.length);
ok('all Skemfar cards uncollectible', pool.every(c => c.collectible === false));
ok('all Skemfar cards dual-color', pool.every(c => JSON.stringify(c.colors) === '["B","G"]'));
ok('all Skemfar cards tagged landSet Skemfar', pool.every(c => c.landSet === 'Skemfar'));

// ---- land tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.skemfar_citadel;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Skemfar');
ok('land tap keys off landSet Skemfar', tapEff && tapEff.landSet === 'Skemfar', JSON.stringify(tapEff));
...
// ---- discover from the land pool offers only Skemfar cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Skemfar' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Skemfar cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Skemfar'), pend && pend.ids);
}
