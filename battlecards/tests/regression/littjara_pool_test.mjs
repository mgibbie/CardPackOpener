// Littjara Citadel advanced-land pool: 15 dual-color, uncollectible "Littjara" cards the land
// Discovers from (keyed by landSet:"Littjara", NOT name-match — the name-match conjure path
...
const temple = byId.littjara_panorama;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Littjara');
ok('land tap keys off landSet Littjara', tapEff && tapEff.landSet === 'Littjara', JSON.stringify(tapEff));
...
// ---- discover from the land pool offers only Littjara cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Littjara' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Littjara cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Littjara'), pend && pend.ids);
}
