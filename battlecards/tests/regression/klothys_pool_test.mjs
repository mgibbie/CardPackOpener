// Temple of Klothys advanced-land pool: 15 mono-white, uncollectible "Klothys" cards the
// land Discovers from (keyed by landSet:"Klothys", NOT name-match — the name-match conjure
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Klothys' && !c.token);
ok('at least 15 Klothys pool cards', pool.length >= 15, pool.length);
ok('all Klothys cards uncollectible', pool.every(c => c.collectible === false));
ok('all Klothys cards dual-color', pool.every(c => JSON.stringify(c.colors) === '[\"R\",\"G\"]'));
ok('all Klothys names contain "Klothys"', pool.every(c => c.name.includes('Klothys')),
	pool.filter(c => !c.name.includes('Klothys')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.temple_of_klothys;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Klothys');
ok('temple tap keys off landSet Klothys', tapEff && tapEff.landSet === 'Klothys', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Klothys cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Klothys' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Klothys cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Klothys'), pend && pend.ids);
}
