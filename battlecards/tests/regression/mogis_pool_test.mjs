// Temple of Mogis advanced-land pool: 15 mono-white, uncollectible "Mogis" cards the
// land Discovers from (keyed by landSet:"Mogis", NOT name-match — the name-match conjure
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Mogis' && !c.token);
ok('at least 15 Mogis pool cards', pool.length >= 15, pool.length);
ok('all Mogis cards uncollectible', pool.every(c => c.collectible === false));
ok('all Mogis cards dual-color', pool.every(c => JSON.stringify(c.colors) === '[\"B\",\"R\"]'));
ok('all Mogis names contain "Mogis"', pool.every(c => c.name.includes('Mogis')),
	pool.filter(c => !c.name.includes('Mogis')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.temple_of_mogis;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Mogis');
ok('temple tap keys off landSet Mogis', tapEff && tapEff.landSet === 'Mogis', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Mogis cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Mogis' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Mogis cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Mogis'), pend && pend.ids);
}
