// Surtland Citadel advanced-land pool: 15 dual-color, uncollectible "Surtland" cards the land
// Discovers from (keyed by landSet:"Surtland", NOT name-match — the name-match conjure path
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Surtland' && !c.token);
ok('at least 15 Surtland pool cards', pool.length >= 15, pool.length);
ok('all Surtland cards uncollectible', pool.every(c => c.collectible === false));
ok('all Surtland cards dual-color', pool.every(c => JSON.stringify(c.colors) === '["U","R"]'));
ok('all Surtland cards tagged landSet Surtland', pool.every(c => c.landSet === 'Surtland'));

// ---- land tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.surtland_citadel;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Surtland');
ok('land tap keys off landSet Surtland', tapEff && tapEff.landSet === 'Surtland', JSON.stringify(tapEff));
...
// ---- discover from the land pool offers only Surtland cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Surtland' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Surtland cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Surtland'), pend && pend.ids);
}
