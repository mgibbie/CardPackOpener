// Temple of Boros advanced-land pool: 15 mono-white, uncollectible "Boros" cards the
// land Discovers from (keyed by landSet:"Boros", NOT name-match — the name-match conjure
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Boros' && !c.token);
ok('at least 15 Boros pool cards', pool.length >= 15, pool.length);
ok('all Boros cards uncollectible', pool.every(c => c.collectible === false));
ok('all Boros cards are R/W dual-color', pool.every(c => JSON.stringify(c.colors) === '["R","W"]'));
ok('all Boros names contain "Boros"', pool.every(c => c.name.includes('Boros')),
	pool.filter(c => !c.name.includes('Boros')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.boros_guildgate;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Boros');
ok('temple tap keys off landSet Boros', tapEff && tapEff.landSet === 'Boros', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Boros cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Boros' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Boros cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Boros'), pend && pend.ids);
}
