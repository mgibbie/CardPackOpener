// Indatha tri-color advanced-land pool: three-color, uncollectible "Indatha" cards the land
// Discovers from (keyed by landSet:"Indatha"). Pools are a 15-card MINIMUM, not a cap.
import fs from 'fs';
...
const pool = raw.cards.filter(c => c.landSet === 'Indatha' && !c.token);
ok('at least 15 Indatha pool cards', pool.length >= 15, pool.length);
ok('all Indatha cards uncollectible', pool.every(c => c.collectible === false));
ok('all Indatha cards tri-color', pool.every(c => JSON.stringify(c.colors) === '["W","B","G"]'));
ok('all Indatha cards tagged landSet Indatha', pool.every(c => c.landSet === 'Indatha'));

const temple = byId.indatha_triome;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Indatha');
ok('land tap keys off landSet Indatha', tapEff && tapEff.landSet === 'Indatha', JSON.stringify(tapEff));
...
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Indatha' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Indatha cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Indatha'), pend && pend.ids);
}
