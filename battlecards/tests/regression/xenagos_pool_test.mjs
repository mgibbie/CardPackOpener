// Temple of Xenagos advanced-land pool: 15 mono-white, uncollectible "Xenagos" cards the
// land Discovers from (keyed by landSet:"Xenagos", NOT name-match — the name-match conjure
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Xenagos' && !c.token);
ok('at least 15 Xenagos pool cards', pool.length >= 15, pool.length);
ok('all Xenagos cards uncollectible', pool.every(c => c.collectible === false));
ok('all Xenagos cards dual-color', pool.every(c => JSON.stringify(c.colors) === '[\"R\",\"G\"]'));
ok('all Xenagos names contain "Xenagos"', pool.every(c => c.name.includes('Xenagos')),
	pool.filter(c => !c.name.includes('Xenagos')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.temple_of_xenagos;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Xenagos');
ok('temple tap keys off landSet Xenagos', tapEff && tapEff.landSet === 'Xenagos', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Xenagos cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Xenagos' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Xenagos cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Xenagos'), pend && pend.ids);
}
