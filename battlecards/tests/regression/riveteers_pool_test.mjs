// Riveteers tri-color advanced-land pool: three-color, uncollectible "Riveteers" cards the land
// Discovers from (keyed by landSet:"Riveteers"). Pools are a 15-card MINIMUM, not a cap.
import fs from 'fs';
...
const pool = raw.cards.filter(c => c.landSet === 'Riveteers' && !c.token);
ok('at least 15 Riveteers pool cards', pool.length >= 15, pool.length);
ok('all Riveteers cards uncollectible', pool.every(c => c.collectible === false));
ok('all Riveteers cards tri-color', pool.every(c => JSON.stringify(c.colors) === '["B","R","G"]'));
ok('all Riveteers cards tagged landSet Riveteers', pool.every(c => c.landSet === 'Riveteers'));

const temple = byId.riveteers_hideout;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Riveteers');
ok('land tap keys off landSet Riveteers', tapEff && tapEff.landSet === 'Riveteers', JSON.stringify(tapEff));
...
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Riveteers' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Riveteers cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Riveteers'), pend && pend.ids);
}
