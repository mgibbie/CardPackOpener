// Obscura tri-color advanced-land pool: three-color, uncollectible "Obscura" cards the land
// Discovers from (keyed by landSet:"Obscura"). Pools are a 15-card MINIMUM, not a cap.
import fs from 'fs';
...
const pool = raw.cards.filter(c => c.landSet === 'Obscura' && !c.token);
ok('at least 15 Obscura pool cards', pool.length >= 15, pool.length);
ok('all Obscura cards uncollectible', pool.every(c => c.collectible === false));
ok('all Obscura cards tri-color', pool.every(c => JSON.stringify(c.colors) === '["W","U","B"]'));
ok('all Obscura cards tagged landSet Obscura', pool.every(c => c.landSet === 'Obscura'));

const temple = byId.obscura_headquarters;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Obscura');
ok('land tap keys off landSet Obscura', tapEff && tapEff.landSet === 'Obscura', JSON.stringify(tapEff));
...
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Obscura' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Obscura cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Obscura'), pend && pend.ids);
}
