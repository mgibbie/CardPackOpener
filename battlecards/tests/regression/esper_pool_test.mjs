// Esper tri-color advanced-land pool: 15 three-color, uncollectible "Esper" cards the land
// Discovers from (keyed by landSet:"Esper", NOT name-match — the name-match conjure path
...
const temple = byId.esper_panorama;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Esper');
ok('land tap keys off landSet Esper', tapEff && tapEff.landSet === 'Esper', JSON.stringify(tapEff));
...
// ---- discover from the land pool offers only Esper cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Esper' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Esper cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Esper'), pend && pend.ids);
}
