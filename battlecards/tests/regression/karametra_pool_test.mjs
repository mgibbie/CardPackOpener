// Temple of Karametra advanced-land pool: 15 mono-white, uncollectible "Karametra" cards the
// land Discovers from (keyed by landSet:"Karametra", NOT name-match — the name-match conjure
...
ok('all Karametra names contain "Karametra"', pool.every(c => c.name.includes('Karametra')),
	pool.filter(c => !c.name.includes('Karametra')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.temple_of_karametra;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Karametra');
ok('temple tap keys off landSet Karametra', tapEff && tapEff.landSet === 'Karametra', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Karametra cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Karametra' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Karametra cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Karametra'), pend && pend.ids);
}
