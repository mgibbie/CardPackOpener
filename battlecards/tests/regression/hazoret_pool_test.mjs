// Temple of Hazoret advanced-land pool: 15 mono-white, uncollectible "Hazoret" cards the
// land Discovers from (keyed by landSet:"Hazoret", NOT name-match — the name-match conjure
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Hazoret' && !c.token);
ok('at least 15 Hazoret pool cards', pool.length >= 15, pool.length);
ok('all Hazoret cards uncollectible', pool.every(c => c.collectible === false));
ok('all Hazoret cards mono-white', pool.every(c => JSON.stringify(c.colors) === '["R"]'));
ok('all Hazoret names contain "Hazoret"', pool.every(c => c.name.includes('Hazoret')),
	pool.filter(c => !c.name.includes('Hazoret')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.temple_of_hazoret;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Hazoret');
ok('temple tap keys off landSet Hazoret', tapEff && tapEff.landSet === 'Hazoret', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Hazoret cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Hazoret' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Hazoret cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Hazoret'), pend && pend.ids);
}
