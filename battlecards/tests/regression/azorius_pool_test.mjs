// Temple of Azorius advanced-land pool: 15 mono-white, uncollectible "Azorius" cards the
// land Discovers from (keyed by landSet:"Azorius", NOT name-match — the name-match conjure
...
ok('all Azorius names contain "Azorius"', pool.every(c => c.name.includes('Azorius')),
	pool.filter(c => !c.name.includes('Azorius')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.azorius_guildgate;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Azorius');
ok('temple tap keys off landSet Azorius', tapEff && tapEff.landSet === 'Azorius', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Azorius cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Azorius' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Azorius cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Azorius'), pend && pend.ids);
}
