// Mardu tri-color advanced-land pool: 15 three-color, uncollectible "Mardu" cards the land
// Discovers from (keyed by landSet:"Mardu", NOT name-match — the name-match conjure path
...
const temple = byId.mardu_outpost;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Mardu');
ok('land tap keys off landSet Mardu', tapEff && tapEff.landSet === 'Mardu', JSON.stringify(tapEff));
...
// ---- discover from the land pool offers only Mardu cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Mardu' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Mardu cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Mardu'), pend && pend.ids);
}
