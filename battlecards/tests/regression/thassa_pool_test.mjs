// Temple of Thassa advanced-land pool: 15 mono-white, uncollectible "Thassa" cards the
// land Discovers from (keyed by landSet:"Thassa", NOT name-match — the name-match conjure
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Thassa' && !c.token);
ok('at least 15 Thassa pool cards', pool.length >= 15, pool.length);
ok('all Thassa cards uncollectible', pool.every(c => c.collectible === false));
ok('all Thassa cards mono-white', pool.every(c => JSON.stringify(c.colors) === '["U"]'));
ok('all Thassa cards tagged landSet Thassa', pool.every(c => c.landSet === 'Thassa'));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.temple_of_thassa;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Thassa');
ok('temple tap keys off landSet Thassa', tapEff && tapEff.landSet === 'Thassa', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Thassa cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Thassa' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Thassa cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Thassa'), pend && pend.ids);
}
