// Temple of Dromoka advanced-land pool: 15 mono-white, uncollectible "Dromoka" cards the
// land Discovers from (keyed by landSet:"Dromoka", NOT name-match — the name-match conjure
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Dromoka' && !c.token);
ok('at least 15 Dromoka pool cards', pool.length >= 15, pool.length);
ok('all Dromoka cards uncollectible', pool.every(c => c.collectible === false));
ok('all Dromoka cards dual-color', pool.every(c => JSON.stringify(c.colors) === '[\"G\",\"W\"]'));
ok('all Dromoka names contain "Dromoka"', pool.every(c => c.name.includes('Dromoka')),
	pool.filter(c => !c.name.includes('Dromoka')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.dromoka_dragonspire;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Dromoka');
ok('temple tap keys off landSet Dromoka', tapEff && tapEff.landSet === 'Dromoka', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Dromoka cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Dromoka' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Dromoka cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Dromoka'), pend && pend.ids);
}
