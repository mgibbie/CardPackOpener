// Temple of Rakdos advanced-land pool: 15 mono-white, uncollectible "Rakdos" cards the
// land Discovers from (keyed by landSet:"Rakdos", NOT name-match — the name-match conjure
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Rakdos' && !c.token);
ok('at least 15 Rakdos pool cards', pool.length >= 15, pool.length);
ok('all Rakdos cards uncollectible', pool.every(c => c.collectible === false));
ok('all Rakdos cards are B/R dual-color', pool.every(c => JSON.stringify(c.colors) === '["B","R"]'));
ok('all Rakdos names contain "Rakdos"', pool.every(c => c.name.includes('Rakdos')),
	pool.filter(c => !c.name.includes('Rakdos')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.rakdos_guildgate;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Rakdos');
ok('temple tap keys off landSet Rakdos', tapEff && tapEff.landSet === 'Rakdos', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Rakdos cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Rakdos' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Rakdos cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Rakdos'), pend && pend.ids);
}
