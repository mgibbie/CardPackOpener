// Temple of Purphoros advanced-land pool: 15 mono-white, uncollectible "Purphoros" cards the
// land Discovers from (keyed by landSet:"Purphoros", NOT name-match — the name-match conjure
...
ok('all Purphoros names contain "Purphoros"', pool.every(c => c.name.includes('Purphoros')),
	pool.filter(c => !c.name.includes('Purphoros')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.temple_of_purphoros;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Purphoros');
ok('temple tap keys off landSet Purphoros', tapEff && tapEff.landSet === 'Purphoros', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Purphoros cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Purphoros' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Purphoros cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Purphoros'), pend && pend.ids);
}
