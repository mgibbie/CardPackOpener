// Temur tri-color advanced-land pool: 15 three-color, uncollectible "Temur" cards the land
// Discovers from (keyed by landSet:"Temur", NOT name-match — the name-match conjure path
...
const temple = byId.temur_outpost;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Temur');
ok('land tap keys off landSet Temur', tapEff && tapEff.landSet === 'Temur', JSON.stringify(tapEff));
...
// ---- discover from the land pool offers only Temur cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Temur' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Temur cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Temur'), pend && pend.ids);
}
