// Temple of Witherbloom advanced-land pool: 15 mono-white, uncollectible "Witherbloom" cards the
// land Discovers from (keyed by landSet:"Witherbloom", NOT name-match — the name-match conjure
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Witherbloom' && !c.token);
ok('at least 15 Witherbloom pool cards', pool.length >= 15, pool.length);
ok('all Witherbloom cards uncollectible', pool.every(c => c.collectible === false));
ok('all Witherbloom cards dual-color', pool.every(c => JSON.stringify(c.colors) === '[\"B\",\"G\"]'));
ok('all Witherbloom cards tagged landSet Witherbloom', pool.every(c => c.landSet === 'Witherbloom'));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.witherbloom_campus;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Witherbloom');
ok('temple tap keys off landSet Witherbloom', tapEff && tapEff.landSet === 'Witherbloom', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Witherbloom cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Witherbloom' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Witherbloom cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Witherbloom'), pend && pend.ids);
}
