// Temple of Orzhov advanced-land pool: 15 mono-white, uncollectible "Orzhov" cards the
// land Discovers from (keyed by landSet:"Orzhov", NOT name-match — the name-match conjure
...
ok('all Orzhov names contain "Orzhov"', pool.every(c => c.name.includes('Orzhov')),
	pool.filter(c => !c.name.includes('Orzhov')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.orzhov_guildgate;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Orzhov');
ok('temple tap keys off landSet Orzhov', tapEff && tapEff.landSet === 'Orzhov', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Orzhov cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Orzhov' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Orzhov cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Orzhov'), pend && pend.ids);
}
