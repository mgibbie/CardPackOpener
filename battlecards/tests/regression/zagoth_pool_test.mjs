// Zagoth tri-color advanced-land pool: three-color, uncollectible "Zagoth" cards the land
// Discovers from (keyed by landSet:"Zagoth"). Pools are a 15-card MINIMUM, not a cap.
import fs from 'fs';
...
const pool = raw.cards.filter(c => c.landSet === 'Zagoth' && !c.token);
ok('at least 15 Zagoth pool cards', pool.length >= 15, pool.length);
ok('all Zagoth cards uncollectible', pool.every(c => c.collectible === false));
ok('all Zagoth cards tri-color', pool.every(c => JSON.stringify(c.colors) === '["U","B","G"]'));
ok('all Zagoth cards tagged landSet Zagoth', pool.every(c => c.landSet === 'Zagoth'));

const temple = byId.zagoth_triome;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Zagoth');
ok('land tap keys off landSet Zagoth', tapEff && tapEff.landSet === 'Zagoth', JSON.stringify(tapEff));
...
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Zagoth' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Zagoth cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Zagoth'), pend && pend.ids);
}
