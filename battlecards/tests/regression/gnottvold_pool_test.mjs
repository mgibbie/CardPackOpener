// Gnottvold Citadel advanced-land pool: 15 dual-color, uncollectible "Gnottvold" cards the land
// Discovers from (keyed by landSet:"Gnottvold", NOT name-match — the name-match conjure path
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Gnottvold' && !c.token);
ok('at least 15 Gnottvold pool cards', pool.length >= 15, pool.length);
ok('all Gnottvold cards uncollectible', pool.every(c => c.collectible === false));
ok('all Gnottvold cards dual-color', pool.every(c => JSON.stringify(c.colors) === '["R","G"]'));
ok('all Gnottvold cards tagged landSet Gnottvold', pool.every(c => c.landSet === 'Gnottvold'));

// ---- land tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.gnottvold_citadel;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Gnottvold');
ok('land tap keys off landSet Gnottvold', tapEff && tapEff.landSet === 'Gnottvold', JSON.stringify(tapEff));
...
// ---- discover from the land pool offers only Gnottvold cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Gnottvold' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Gnottvold cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Gnottvold'), pend && pend.ids);
}
