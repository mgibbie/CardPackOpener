// Temple of Silverquill advanced-land pool: 15 mono-white, uncollectible "Silverquill" cards the
// land Discovers from (keyed by landSet:"Silverquill", NOT name-match — the name-match conjure
...
const temple = byId.silverquill_campus;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Silverquill');
ok('temple tap keys off landSet Silverquill', tapEff && tapEff.landSet === 'Silverquill', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Silverquill cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Silverquill' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Silverquill cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Silverquill'), pend && pend.ids);
}
