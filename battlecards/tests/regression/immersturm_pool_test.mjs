// Immersturm Citadel advanced-land pool: 15 dual-color, uncollectible "Immersturm" cards the land
// Discovers from (keyed by landSet:"Immersturm", NOT name-match — the name-match conjure path
...
const temple = byId.immersturm_citadel;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Immersturm');
ok('land tap keys off landSet Immersturm', tapEff && tapEff.landSet === 'Immersturm', JSON.stringify(tapEff));
...
// ---- discover from the land pool offers only Immersturm cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Immersturm' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Immersturm cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Immersturm'), pend && pend.ids);
}
