// Axgard Citadel advanced-land pool: 15 dual-color, uncollectible "Axgard" cards the land
// Discovers from (keyed by landSet:"Axgard", NOT name-match — the name-match conjure path
...
const temple = byId.axgard_citadel;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Axgard');
ok('land tap keys off landSet Axgard', tapEff && tapEff.landSet === 'Axgard', JSON.stringify(tapEff));
...
// ---- discover from the land pool offers only Axgard cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Axgard' }], null, null);
	const pend = st.pickQueue[0];
	ok('land discover queues a pick', !!pend);
	ok('discover offers only Axgard cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Axgard'), pend && pend.ids);
}
