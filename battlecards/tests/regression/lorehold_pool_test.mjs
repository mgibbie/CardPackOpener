// Temple of Lorehold advanced-land pool: 15 mono-white, uncollectible "Lorehold" cards the
// land Discovers from (keyed by landSet:"Lorehold", NOT name-match — the name-match conjure
...
const temple = byId.lorehold_campus;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Lorehold');
ok('temple tap keys off landSet Lorehold', tapEff && tapEff.landSet === 'Lorehold', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Lorehold cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Lorehold' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Lorehold cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Lorehold'), pend && pend.ids);
}
