...
// a hero power goes on the stack whenever the (randomly dealt) opponent holds a
// possible response — drain it so these checks measure the power, not the deal
const settle = (st) => {
...
// Non-rogue (Mage): Poisoned Blade still gains +1 even though the Hero Power doesn't equip
{
	...
	E.useHeroPower(st, 0, hp.uid, { type: 'hero', player: 1 }, null); settle(st);
