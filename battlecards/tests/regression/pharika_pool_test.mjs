// Temple of Pharika advanced-land pool: 15 mono-white, uncollectible "Pharika" cards the
// land Discovers from (keyed by landSet:"Pharika", NOT name-match — the name-match conjure
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Pharika' && !c.token);
ok('at least 15 Pharika pool cards', pool.length >= 15, pool.length);
ok('all Pharika cards uncollectible', pool.every(c => c.collectible === false));
ok('all Pharika cards dual-color', pool.every(c => JSON.stringify(c.colors) === '[\"B\",\"G\"]'));
ok('all Pharika names contain "Pharika"', pool.every(c => c.name.includes('Pharika')),
	pool.filter(c => !c.name.includes('Pharika')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.temple_of_pharika;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Pharika');
ok('temple tap keys off landSet Pharika', tapEff && tapEff.landSet === 'Pharika', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Pharika cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Pharika' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Pharika cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Pharika'), pend && pend.ids);
}
