// Temple of Golgari advanced-land pool: 15 mono-white, uncollectible "Golgari" cards the
// land Discovers from (keyed by landSet:"Golgari", NOT name-match — the name-match conjure
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Golgari' && !c.token);
ok('at least 15 Golgari pool cards', pool.length >= 15, pool.length);
ok('all Golgari cards uncollectible', pool.every(c => c.collectible === false));
ok('all Golgari cards are B/G dual-color', pool.every(c => JSON.stringify(c.colors) === '["B","G"]'));
ok('all Golgari names contain "Golgari"', pool.every(c => c.name.includes('Golgari')),
	pool.filter(c => !c.name.includes('Golgari')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.golgari_guildgate;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Golgari');
ok('temple tap keys off landSet Golgari', tapEff && tapEff.landSet === 'Golgari', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Golgari cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Golgari' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Golgari cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Golgari'), pend && pend.ids);
}
