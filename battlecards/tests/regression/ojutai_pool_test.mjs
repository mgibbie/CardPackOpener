// Temple of Ojutai advanced-land pool: 15 mono-white, uncollectible "Ojutai" cards the
// land Discovers from (keyed by landSet:"Ojutai", NOT name-match — the name-match conjure
...
// ---- pool membership ----
const pool = raw.cards.filter(c => c.landSet === 'Ojutai' && !c.token);
ok('at least 15 Ojutai pool cards', pool.length >= 15, pool.length);
ok('all Ojutai cards uncollectible', pool.every(c => c.collectible === false));
ok('all Ojutai cards dual-color', pool.every(c => JSON.stringify(c.colors) === '[\"W\",\"U\"]'));
ok('all Ojutai names contain "Ojutai"', pool.every(c => c.name.includes('Ojutai')),
	pool.filter(c => !c.name.includes('Ojutai')).map(c => c.name));

// ---- temple tap surfaces the pool via landSet (not the collectible-filtering name-match) ----
const temple = byId.ojutai_dragonspire;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Ojutai');
ok('temple tap keys off landSet Ojutai', tapEff && tapEff.landSet === 'Ojutai', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Ojutai cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Ojutai' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Ojutai cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Ojutai'), pend && pend.ids);
}
