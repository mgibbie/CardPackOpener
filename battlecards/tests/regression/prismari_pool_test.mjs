// Temple of Prismari advanced-land pool: 15 mono-white, uncollectible "Prismari" cards the
// land Discovers from (keyed by landSet:"Prismari", NOT name-match — the name-match conjure
...
const temple = byId.prismari_campus;
const tapEff = temple.taps.flatMap(t => t.effects).find(e => e.landSet === 'Prismari');
ok('temple tap keys off landSet Prismari', tapEff && tapEff.landSet === 'Prismari', JSON.stringify(tapEff));
...
// ---- discover from the temple pool offers only Prismari cards ----
{
	const st = fresh();
	E.execEffects(st, 0, [{ type: 'discover', landSet: 'Prismari' }], null, null);
	const pend = st.pickQueue[0];
	ok('temple discover queues a pick', !!pend);
	ok('discover offers only Prismari cards', pend && pend.ids && pend.ids.every(id => byId[id] && byId[id].landSet === 'Prismari'), pend && pend.ids);
}
