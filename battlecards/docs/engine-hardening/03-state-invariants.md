...
| I6 | Hand size ≤ HAND_LIMIT (15) **at turn boundaries only** (MAX_HAND is a high mid-turn safety ceiling, not the real limit) — overflow is legal mid-turn ("No burn on overdraw", enforced by cleanup discard queue) | Soft by design. Validator must check only after `endTurn` resolution, and account for `discardQueue` pending. |
