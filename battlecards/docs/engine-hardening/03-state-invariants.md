...
| I15 | Every `e.type` reaching a dispatcher has a handler | **Silently violated**: unknown types fall through the 935-branch chain and do nothing. A dev-mode "unknown effect" detector is one of the cheapest, highest-value diagnostics available (see Phase 1). |
