...
function execEffects(state, pi, effects, target, source) {
	...
	for (const e of effects || []) {
		...
		if (e.type === 'damage') {
			...
			const schoolImmune = t2 => t2 && t2.immuneToSchool && source && isSpellType(source) && schoolOf(source) === t2.immuneToSchool;
			switch (e.target) {
