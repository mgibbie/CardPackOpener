...
const FRIENDLY_TYPES = new Set(['buff', 'boost', 'grant', 'temp-buff', 'heal-full', 'attack-equals-health', 'double-health', 'double-attack', 'grant-ongoing', 'temp-immune', 'shadowflame', 'grant-deathrattle', 'copy-deathrattle', 'attach', 'add-counters', 'grow', 'silent-strike']);
const HOSTILE_TYPES = new Set(['damage', 'destroy', 'exile', 'set-health', 'set-attack', 'bounce', 'mind-control', 'transform', 'transform-copy', 'damage-then', 'conditional', 'draw-damage', 'swap-stats', 'swipe', 'damage-adjacent', 'betrayal', 'corrupt', 'mind-control-temp', 'attach-curse', 'tricks-of-the-trade']);
...
export function step(state, pi = 1) {
	...
	if (state.pickQueue.length && state.pickQueue[0].player === pi) {
		...
		if (pend.advance) { E.resolvePick(state, pend.ids[0]); return true; } // dungeon/room choice: take the first
		if (pend.mode === 'adapt') {
