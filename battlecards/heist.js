...
export const PASSIVES = {
	recycling: {
		name: 'Recycling', text: 'After a friendly creature dies, gain 2 Armor.',
		apply: (state, pi) => { const p = state.players[pi]; p.armorPerFriendlyDeath = (p.armorPerFriendlyDeath || 0) + 2; },
	},
	rocket_backpacks: {
		name: 'Rocket Backpacks', text: 'Your creatures have Rush.',
		apply: (state, pi) => {
			const em = E.instantiate({ id: 'heist_rocket_backpacks', name: 'Rocket Backpacks', type: 'emblem', cost: 0, rarity: 'basic', description: 'Your creatures have Rush.', aura: { keywords: ['rush'] } }, pi);
			em.zone = 'emblem';
	...
	resourcefulness: {
		name: 'Resourcefulness', text: 'At the start of the game, equip a random weapon & give it +1/+1.',
	...
	dr_booms_remote: {
		name: "Dr. Boom's Remote", text: 'At the start of the game, create three 1/1 Boom Bots.',
		apply: (state, pi) => { for (let i = 0; i < 3; i++) if (state.cardsById['boom_bot']) E.summon(state, pi, state.cardsById['boom_bot']); },
	},
	hagathas_embrace: {
		name: "Hagatha's Embrace", text: 'At the start of your turn, give a random creature in your hand +1/+1.',
...
export const BOSSES = {
	chomper: { name: 'Chomper', health: 10, hsId: 'DALA_BOSS_01h', power: { name: 'Chomp', cost: 2, text: 'Deal 1 damage to a creature. Lifesteal.', effects: [{ type: 'damage', value: 1, target: 'creature', lifesteal: true }] }, theme: { tribe: 'Beast', maxCost: 4 } },
	awilo: { name: 'Awilo, Cooking Trainer', health: 30, hsId: 'DALA_BOSS_02h', power: { name: 'The Feast', cost: 1, text: 'Gain 3 life.', effects: [{ type: 'heal', value: 3, target: 'self' }] }, theme: { cardClass: 'priest' } },
	...
	kaye_toogie: { name: 'Kaye Toogie', health: 30, hsId: 'DALA_BOSS_04h', power: { name: 'Open Wormhole', cost: 2, text: 'Summon a creature from your deck.', effects: [{ type: 'summon-random-from-deck' }] }, theme: { cardClass: 'warlock' } },
	...
	applebough: { name: 'Applebough', health: 30, hsId: 'DALA_BOSS_08h', power: { name: 'Apple Toss', cost: 2, text: 'Deal 2 damage to a random enemy creature.', effects: [{ type: 'random-damage', value: 2, count: 1, pool: 'enemy-creatures' }] }, theme: { cardClass: 'druid' } },
	archivist_oshi: { name: 'Archivist Oshi', health: 30, hsId: 'DALA_BOSS_09h', power: { name: 'Repeat History', cost: 2, text: 'Resurrect your strongest dead creature.', effects: [{ type: 'resurrect-highest-died' }] }, theme: { cardClass: 'priest' } },
	...
	noz_timbertail: { name: 'Noz Timbertail', health: 30, hsId: 'DALA_BOSS_11h', power: { name: 'Backstabber', cost: 2, text: 'Give a random friendly creature +1/+1 & Stealth.', effects: [{ type: 'buff-random-friendly', attack: 1, health: 1, grant: 'stealth' }] }, theme: { cardClass: 'rogue' } },
	cravitz_lorent: { name: 'Cravitz Lorent', health: 30, hsId: 'DALA_BOSS_12h', power: { name: 'Forbidden Love', cost: 3, text: 'Destroy a random enemy creature.', effects: [{ type: 'destroy-random' }] }, theme: { cardClass: 'warlock' } },
	albin_eastoft: { name: 'Albin Eastoft', health: 30, hsId: 'DALA_BOSS_13h', power: { name: 'Slice and Dice', cost: 2, text: 'Deal 1 damage to three random enemy creatures.', effects: [{ type: 'random-damage', value: 1, count: 3, pool: 'enemy-creatures' }] }, theme: { cardClass: 'rogue' } },
	tierra_blythe: { name: 'Tierra Blythe', health: 30, hsId: 'DALA_BOSS_14h', power: { name: 'Order Up!', cost: 2, text: 'Create a 2/2 Kind Waitress.', effects: [{ type: 'summon', count: 1, attack: 2, health: 2, name: 'Kind Waitress' }] }, theme: { cardClass: 'neutral', maxCost: 5 } },
	moon_priestess_nici: { name: 'Moon Priestess Nici', health: 30, hsId: 'DALA_BOSS_15h', power: { name: 'Blessing of Elune', cost: 1, text: 'Gain 4 life.', effects: [{ type: 'heal', value: 4, target: 'self' }] }, theme: { cardClass: 'priest' } },
	...
	tala_stonerage: { name: 'Tala Stonerage', health: 30, hsId: 'DALA_BOSS_17h', power: { name: 'Twin Paths', cost: 2, text: 'Give your creatures +1/+1.', effects: [{ type: 'buff', attack: 1, health: 1, target: 'friendly-creatures' }] }, theme: { cardClass: 'druid' } },
	disidra_stormglory: { name: 'Disidra Stormglory', health: 30, hsId: 'DALA_BOSS_18h', power: { name: 'Totemic Summons', cost: 2, text: 'Create a random Totem.', effects: [{ type: 'summon-random', tribe: 'Totem' }] }, theme: { cardClass: 'shaman' } },
	...
	bookmaster_bae_chao: { name: 'Bookmaster Bae Chao', health: 30, hsId: 'DALA_BOSS_22h', power: { name: 'Shhh!', cost: 3, text: 'Silence all enemy creatures.', effects: [{ type: 'silence', target: 'enemy-creatures' }] }, theme: { cardClass: 'priest' } },
	sharky_mcfin: { name: 'Sharky McFin', health: 30, hsId: 'DALA_BOSS_23h', power: { name: 'Sharkbite', cost: 3, text: 'Destroy a random enemy creature.', effects: [{ type: 'destroy-random' }] }, theme: { tribe: 'Murloc' } },
	ranger_ar_ha: { name: "Ranger Ar'ha", health: 30, hsId: 'DALA_BOSS_24h', power: { name: "Ar'ha's Call", cost: 2, text: 'Give a random friendly creature +2/+2.', effects: [{ type: 'buff-random-friendly', attack: 2, health: 2 }] }, theme: { cardClass: 'hunter' } },
	jepetto_joybuzz: { name: 'Jepetto Joybuzz', health: 30, hsId: 'DALA_BOSS_25h', power: { name: 'Assembly', cost: 2, text: 'Create a 1/1 copy of a random creature in your hand.', effects: [{ type: 'summon-hand-copy', attack: 1, health: 1 }] }, theme: { cardClass: 'neutral' } },
	dalaran_fountain_golem: { name: 'Dalaran Fountain Golem', health: 30, hsId: 'DALA_BOSS_26h', power: { name: 'Cold Water', cost: 1, text: 'Freeze an enemy creature.', effects: [{ type: 'freeze', target: 'enemy-creature' }] }, theme: { tribe: 'Elemental' } },
	rasil_fireborne: { name: 'Rasil Fireborne', health: 30, hsId: 'DALA_BOSS_27h', power: { name: 'Masterpiece!', cost: 3, text: 'Create a random creature that costs (5).', effects: [{ type: 'summon-random', cost: 5 }] }, theme: { tribe: 'Elemental' } },
	...
	the_rat_king: { name: 'The Rat King', health: 10, hsId: 'DALA_BOSS_30h', power: { name: 'A Tale of Kings', cost: 3, text: 'Create a 2/2 Underbelly Rat.', effects: [{ type: 'summon', count: 1, attack: 2, health: 2, name: 'Underbelly Rat', tribe: 'Beast' }] }, theme: { tribe: 'Beast', maxCost: 4 } },
	timothy_jones: { name: 'Timothy Jones', health: 30, hsId: 'DALA_BOSS_31h', power: { name: 'Bling it On!', cost: 2, text: 'Give a random friendly creature +2/+2.', effects: [{ type: 'buff-random-friendly', attack: 2, health: 2 }] }, theme: { cardClass: 'paladin' } },
	kizi_copperclip: { name: 'Kizi Copperclip', health: 10, hsId: 'DALA_BOSS_32h', power: { name: 'Makeover', cost: 1, text: 'Give a random friendly creature +1/+1.', effects: [{ type: 'buff-random-friendly', attack: 1, health: 1 }] }, theme: { cardClass: 'neutral', maxCost: 3 } },
	...
	carousel_gryphon: { name: 'Carousel Gryphon', health: 30, hsId: 'DALA_BOSS_34h', power: { name: 'Merry Go Round', cost: 2, text: 'Give your creatures +1/+1.', effects: [{ type: 'buff', attack: 1, health: 1, target: 'friendly-creatures' }] }, theme: { tribe: 'Mech' } },
	soothsayer_zoie: { name: 'Soothsayer Zoie', health: 30, hsId: 'DALA_BOSS_35h', power: { name: 'Healing Hands', cost: 1, text: 'Gain 3 life.', effects: [{ type: 'heal', value: 3, target: 'self' }] }, theme: { cardClass: 'priest' } },
	draemus: { name: 'Draemus', health: 30, hsId: 'DALA_BOSS_36h', power: { name: 'Import Pet', cost: 2, text: 'Create a random Beast that costs (3).', effects: [{ type: 'summon-random', tribe: 'Beast', cost: 3 }] }, theme: { tribe: 'Beast' } },
	...
	aki_the_brilliant: { name: 'Aki the Brilliant', health: 30, hsId: 'DALA_BOSS_39h', power: { name: 'For The Light!', cost: 2, text: 'Give all creatures in your hand +1/+1.', effects: [{ type: 'buff-hand', all: true, attack: 1, health: 1 }] }, theme: { cardClass: 'paladin' } },
	oxana_demonslay: { name: 'Oxana Demonslay', health: 30, hsId: 'DALA_BOSS_40h', power: { name: 'Immolation Aura', cost: 2, text: 'Deal 2 damage to a random enemy creature.', effects: [{ type: 'random-damage', value: 2, count: 1, pool: 'enemy-creatures' }] }, theme: { cardClass: 'demon_hunter' } },
	ungan_oddkind: { name: 'Ungan Oddkind', health: 30, hsId: 'DALA_BOSS_41h', power: { name: 'Summon Companion', cost: 4, text: 'Create a random Animal Companion.', effects: [{ type: 'summon', count: 1, options: [{ summonId: 'huffer' }, { summonId: 'leokk' }, { summonId: 'misha' }] }] }, theme: { cardClass: 'hunter' } },
	p_o_g_o: { name: 'P.O.G.O.', health: 30, hsId: 'DALA_BOSS_42h', power: { name: 'Pogoshuffle', cost: 2, text: 'Shuffle 3 copies of a friendly creature into your deck.', effects: [{ type: 'shuffle-copies-of-target', count: 3, target: 'friendly-creature' }] }, theme: { tribe: 'Mech' } },
	...
	erekem: { name: 'Erekem', health: 30, hsId: 'DALA_BOSS_44h', power: { name: 'Dark Tidings', cost: 1, text: 'Draw a creature.', effects: [{ type: 'tutor', cardType: 'creature', count: 1 }] }, theme: { cardClass: 'shaman' } },
	moragg: { name: 'Moragg', health: 30, hsId: 'DALA_BOSS_45h', power: { name: 'Ray of Suffering', cost: 2, text: 'Deal 2 damage to target opponent.', effects: [{ type: 'damage', value: 2, target: 'enemy-hero' }] }, theme: { tribe: 'Demon' } },
	ichoron: { name: 'Ichoron', health: 30, hsId: 'DALA_BOSS_46h', power: { name: 'Protective Bubble', cost: 1, text: 'Give a random friendly creature Divine Shield.', effects: [{ type: 'buff-random-friendly', attack: 0, health: 0, grant: 'divine_shield' }] }, theme: { tribe: 'Elemental' } },
	lavanthor: { name: 'Lavanthor', health: 30, hsId: 'DALA_BOSS_47h', power: { name: 'Lava Belch', cost: 1, text: 'Create a 0/3 Molten Rock with Taunt.', effects: [{ type: 'summon', count: 1, attack: 0, health: 3, name: 'Molten Rock', tribe: 'Elemental', keywords: ['taunt'] }] }, theme: { tribe: 'Elemental' } },
	zuramat: { name: 'Zuramat the Obliterator', health: 30, hsId: 'DALA_BOSS_48h', power: { name: 'To The Void', cost: 2, text: 'Banish an enemy creature.', effects: [{ type: 'exile', target: 'enemy-creature' }] }, theme: { tribe: 'Demon' } },
	cyanigosa: { name: 'Cyanigosa', health: 40, hsId: 'DALA_BOSS_49h', power: { name: 'Uncontrollable Energy', cost: 2, text: 'Deal 2 damage to all enemy creatures.', effects: [{ type: 'damage', value: 2, target: 'enemy-creatures' }] }, theme: { tribe: 'Dragon' } },
	...
	millificent_manastorm: { name: 'Millificent Manastorm', health: 30, hsId: 'DALA_BOSS_51h', power: { name: 'Tinker', cost: 2, text: 'Create a 0/4 Squirrel Bomb with Taunt.', effects: [{ type: 'summon', count: 1, attack: 0, health: 4, name: 'Squirrel Bomb', tribe: 'Mech', keywords: ['taunt'] }] }, theme: { tribe: 'Mech' } },
	lieutenant_sinclari: { name: 'Lieutenant Sinclari', health: 30, hsId: 'DALA_BOSS_52h', power: { name: 'Hold the Gates!', cost: 2, text: 'Give a creature +2 Health & Taunt.', effects: [{ type: 'buff', attack: 0, health: 2, target: 'friendly-creature' }, { type: 'grant', keyword: 'taunt', target: 'friendly-creature' }] }, theme: { cardClass: 'paladin' } },
	dancin_deryl: { name: "Dancin' Deryl", health: 30, hsId: 'DALA_BOSS_53h', power: { name: 'Boogie Woogie', cost: 2, text: 'Give a random friendly creature +2/+2.', effects: [{ type: 'buff-random-friendly', attack: 2, health: 2 }] }, theme: { cardClass: 'neutral' } },
	...
	captain_hannigan: { name: 'Captain Hannigan', health: 30, hsId: 'DALA_BOSS_55h', power: { name: 'Raise the Alarm', cost: 2, text: 'Create a 1/4 Kirin Tor Guard with Taunt.', effects: [{ type: 'summon', count: 1, attack: 1, health: 4, name: 'Kirin Tor Guard', keywords: ['taunt'] }] }, theme: { cardClass: 'paladin' } },
	...
	trade_prince_gallywix: { name: 'Trade Prince Gallywix', health: 40, hsId: 'DALA_BOSS_57h', power: { name: 'Greed is Good', cost: 1, text: "Copy a random card in your opponent's hand & get a Coin.", effects: [{ type: 'copy-random-enemy-hand' }, { type: 'gain-coin' }] }, theme: { cardClass: 'rogue' } },
	boommaster_flark: { name: 'Boommaster Flark', health: 30, hsId: 'DALA_BOSS_58h', power: { name: 'Blast Powder', cost: 2, text: 'Deal 2 damage to a random enemy creature.', effects: [{ type: 'random-damage', value: 2, count: 1, pool: 'enemy-creatures' }] }, theme: { tribe: 'Mech' } },
	...
	archmage_khadgar: { name: 'Archmage Khadgar', health: 40, hsId: 'DALA_BOSS_60h', power: { name: 'Summon Elemental', cost: 2, text: 'Create a 2/3 Elemental.', effects: [{ type: 'summon', count: 1, attack: 2, health: 3, name: 'Servant of Khadgar', tribe: 'Elemental' }] }, theme: { cardClass: 'mage' } },
	...
	haro_setting_sun: { name: 'Haro Setting-Sun', health: 30, hsId: 'DALA_BOSS_62h', power: { name: 'Darken', cost: 2, text: 'Give an enemy creature -2 Attack until your next turn.', effects: [{ type: 'debuff-until-your-next', value: 2, target: 'enemy-creature' }] }, theme: { cardClass: 'priest' } },
	dagg_cruelmight: { name: 'Dagg Cruelmight', health: 30, hsId: 'DALA_BOSS_63h', power: { name: 'Bully', cost: 2, text: 'Deal 2 damage to a damaged creature.', effects: [{ type: 'damage', value: 2, target: 'creature', requireDamaged: true }] }, theme: { cardClass: 'warrior' } },
	banker_biggs: { name: 'Banker Biggs', health: 40, hsId: 'DALA_BOSS_64h', power: { name: 'Invest!', cost: 2, text: 'Give a friendly creature +4/+4.', effects: [{ type: 'buff', attack: 4, health: 4, target: 'friendly-creature' }] }, theme: { cardClass: 'neutral' } },
	...
	lilayell_suntear: { name: 'Lilayell Suntear', health: 30, hsId: 'DALA_BOSS_68h', power: { name: 'Mini Portal', cost: 1, text: 'Create a random 1-Cost creature.', effects: [{ type: 'summon-random', cost: 1 }] }, theme: { cardClass: 'mage' } },
	...
	sky_captain_smiggs: { name: 'Sky Captain Smiggs', health: 30, hsId: 'DALA_BOSS_70h', power: { name: 'Bombardment', cost: 3, text: 'Deal 2 damage to all enemy creatures.', effects: [{ type: 'damage', value: 2, target: 'enemy-creatures' }] }, theme: { tribe: 'Pirate' } },
	anarii_duskgrove: { name: 'Anarii Duskgrove', health: 30, hsId: 'DALA_BOSS_71h', power: { name: 'Summon Protectors', cost: 2, text: 'Create a 2/2 Treant with Taunt.', effects: [{ type: 'summon', count: 1, attack: 2, health: 2, name: 'Treant', keywords: ['taunt'] }] }, theme: { cardClass: 'druid' } },
	...
	hesutu_stonewind: { name: 'Hesutu Stonewind', health: 30, hsId: 'DALA_BOSS_73h', power: { name: "Earthmother's Rage", cost: 2, text: 'Give a friendly creature Windfury.', effects: [{ type: 'grant', keyword: 'windfury', target: 'friendly-creature' }] }, theme: { cardClass: 'shaman' } },
	kara_stamper: { name: 'Kara Stamper', health: 30, hsId: 'DALA_BOSS_74h', power: { name: 'Soul Weave', cost: 3, text: 'Discard a card. Create a random creature that costs (3).', effects: [{ type: 'discard-random', count: 1 }, { type: 'summon-random', cost: 3 }] }, theme: { cardClass: 'warlock' } },
	commander_bolan: { name: 'Commander Bolan', health: 30, hsId: 'DALA_BOSS_75h', power: { name: 'For the Kirin Tor!', cost: 2, text: 'Create a 1/4 Kirin Tor Guard with Taunt.', effects: [{ type: 'summon', count: 1, attack: 1, health: 4, name: 'Kirin Tor Guard', keywords: ['taunt'] }] }, theme: { cardClass: 'paladin' } },
};
...
export const TAVERN = {
	...
	dismiss: {
		name: 'Dismiss', pick: 'deck-creature', text: 'Remove a creature from your deck.',
		apply: (run, id) => { const i = run.deck.indexOf(id); if (i >= 0) run.deck.splice(i, 1); },
	},
	gang: {
		name: "The Gang's All Here!", pick: 'deck-creature', text: 'Add three copies of a creature to your deck.',
		apply: (run, id) => { run.deck.push(id, id, id); },
	},
	veteran: {
		name: 'Recruit a Veteran', pick: 'legendary', text: 'Add a Legendary creature to your deck.',
		apply: (run, id) => { run.deck.push(id); },
	},
	tell_a_story: {
		name: 'Tell a Story', pick: 'deck-creature', text: 'Give a creature in your deck +2/+2 for the heist.',
		apply: (run, id) => { (run.deckBuffs = run.deckBuffs || []).push({ id, attack: 2, health: 2, cost: 0 }); },
	},
	tall_tales: {
		name: 'Tall Tales', pick: 'deck-creature', text: 'Give a creature +4/+4, but it costs (2) more for the heist.',
		apply: (run, id) => { (run.deckBuffs = run.deckBuffs || []).push({ id, attack: 4, health: 4, cost: 2 }); },
	},
	right_hand_man: {
		name: 'Right Hand Man', pick: 'deck-creature', text: 'A creature always starts in your opening hand.',
...
export const ANOMALIES = {
	...
	gorged: {
		name: 'Gorged', text: 'Both players start with two extra cards & Mana Crystals.',
		boot: (state) => { for (let pi = 0; pi < state.players.length; pi++) { E.drawCards(state, pi, 2); E.addManaCrystal(state, pi, 2); } },
	},
	growing: { name: 'Growing', text: "Creatures gain +1/+1 at the end of their owner's turn." },
	rejuvenating: { name: 'Rejuvenating', text: 'At the start of your turn, Gain 2 life.' },
	reductive: { name: 'Reductive', text: 'At the end of your turn, reduce the Cost of cards in your hand by (1).' },
	dragon_soul: { name: 'Dragon Soul', text: 'After you cast 3 spells in a turn, create a 5/5 Dragon.' },
	infused: { name: 'Infused', text: 'Created creatures gain Taunt, Divine Shield, Rush, or Windfury at random.' },
	explosive: { name: 'Explosive', text: 'Created creatures gain "Deathrattle: Deal 1 damage to all creatures."' },
	nesting: { name: 'Nesting', text: 'Created creatures gain "Deathrattle: Create a 1/1 copy of this creature."' },
};
